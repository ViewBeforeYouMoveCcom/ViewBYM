...
export default function PhotoGallery({ images, title }: Props) {
  ...
  const lightbox = open && mounted ? createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/95" onClick={() => setOpen(false)}>
      ...
      {/* Counter */}
      <div className="absolute left-1/2 top-4 -translate-x-1/2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[12px] font-semibold text-white">
    ...
    <>
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        {images.map((src, i) => (
          ...
          >
            <Image src={src} alt={`${title} photo ${i + 1}`} fill className="object-cover transition-transform duration-200 group-hover:scale-105" unoptimized />
