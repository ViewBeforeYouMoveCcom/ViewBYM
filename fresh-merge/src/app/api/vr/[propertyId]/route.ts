...
export async function GET(
  ...
  // Look up the VR record — only platform-hosted video_path, no embed/iframe
  const { data: vr, error } = await supabase
    .from("property_vr")
    .select("video_path, video_url, is_enabled")
  ...
  // Resolve storage path: prefer video_path, fall back to extracting from legacy video_url
  let storagePath: string | null = vr.video_path ?? null;

  if (!storagePath && vr.video_url) {
    const match = (vr.video_url as string).match(/property-vr\/(.+)$/);
