    ...
    async function load() {
      ...
      if (user) {
        ...
        try {
          ...
          const stored = raw ? (JSON.parse(raw) as Array<{ id: string; label?: string; name?: string; filters: Record<string, unknown> }>) : [];
  ...
  return (
    <div className="space-y-4">
      ...
      {searches.length === 0 ? (
        ...
        <div className="space-y-3">
          {searches.map((search) => {
            const summary = search.name ?? filtersToSummary(search.filters);
