...
function AuthCallbackHandler() {
  ...
  useEffect(() => {
    ...
    const fallback = searchParams.get("fallback") ?? "/account/login";

    // If OAuth was cancelled/denied, the error lands in the URL hash or search params
    const urlSearch = new URLSearchParams(window.location.search);
    const urlHash = new URLSearchParams(window.location.hash.slice(1));
    if (urlSearch.get("error") || urlHash.get("error")) {
