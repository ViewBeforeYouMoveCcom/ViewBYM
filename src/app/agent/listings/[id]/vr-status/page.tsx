...
> = {
  not_submitted: {
    ...
    body: "Submit your 360° footage to start the processing workflow. Once uploaded, the VBYM team will process and publish your tour.",
  },
  queued: {
    label: "Queued",
    variant: "amber",
    ...
    async function load() {
      const [{ data: prop }, { data: vrData }] = await Promise.all([
        ...
        supabaseClient
          .from("property_vr")
          .select(
            "id, submission_status, submission_notes, submitted_at, raw_footage_path"
          )
          .eq("property_id", id)
          .maybeSingle(),
      ]);
