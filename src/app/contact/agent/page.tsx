  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await fetch("/api/enquiry", {
        ...
        body: JSON.stringify({ ...data, agentEmail: "jt@universaltvmedia.com", propertyTitle: "Agent / partner enquiry" }),
      });
      setSubmitted(true);
    } catch {
      // silently handle
    ...
    <div className="bg-white font-sans">
      <section className="relative overflow-hidden bg-[#1A3A6C] py-12 sm:py-16">
        <div className="relative mx-auto max-w-[1800px] px-5">
          ...
          <p className="text-[15px] text-white/70">Discuss VR capture, onboarding, or listing integrations — we&apos;ll get back to you promptly.</p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-[700px] px-5">
          {submitted ? (
            <div className="rounded-2xl border border-green-100 bg-green-50 p-8 text-center">
              ...
              <p className="mt-2 text-[14px] text-gray-500">Our team will be in touch with you shortly.</p>
              <Link href="/for-agents" className="mt-6 inline-block rounded-lg bg-[#08519A] px-6 py-2.5 text-[14px] font-semibold text-white hover:bg-[#063d75]">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-8 shadow-sm">
              <form onSubmit={handleSubmit} className="space-y-5">
                <FormField id="name" label="Full name" helper="Include your role and agency name.">
                  <Input id="name" name="name" placeholder="Tom Briggs" required />
                </FormField>
                <FormField id="agency" label="Agency name">
                  <Input id="agency" name="agency" placeholder="Briggs & Co Estate Agents" required />
                </FormField>
                <FormField id="email" label="Business email">
                  <Input id="email" name="email" type="email" placeholder="tom@agency.co.uk" required />
                </FormField>
                ...
                <FormField id="message" label="Message">
                  <Textarea id="message" name="message" rows={5} placeholder="Tell us about your agency, how many listings you have, and how we can support you." required />
                </FormField>
                <Button
                  type="submit"
                  disabled={loading}
