...
export default function HeaderNav() {
  ...
  // First name only — falls back to email prefix if no name set yet
  ...
  return (
    <header className="sticky top-0 z-40 w-full border-b border-gray-200 bg-white/95 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-2.5">
          {!authLoading && user ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                href="/account/signup"
                className="h-9 rounded-lg bg-blue-700 px-4 text-[13.5px] font-semibold text-white transition-colors hover:bg-blue-800 flex items-center"
              >
                Sign up free
              </Link>
            </div>
      ...
      <div className="border-t border-gray-100 lg:hidden">
        <div className="mx-auto flex w-full max-w-[1200px] items-center gap-5 overflow-x-auto px-5 py-2.5">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="whitespace-nowrap text-[13px] font-medium text-gray-600 hover:text-gray-900"
            >
              {item.label}
            </Link>
          ))}
